/*----------------------------------------------------------------------------*/        
/* REL_HISTORICO_TRANSACOES                                 VERSAO 3          */             
...
/* REV04 limitar o tempo para 1 ano no máximo                                 */        
/*----------------------------------------------------------------------------*/         
ALTER PROCEDURE [dbo].[REL_HISTORICO_TRANSACOES] 
            @SISTEMA int = 0, 
            @CODCLI int = 0,   
            @CPF char(11) = null, 
            @DATAINI CHAR(08), 
...
BEGIN  
 ...
 --SET @SISTEMA  = 0  
 --SET @CODCLI   = 99998 --60449  
 --SET @CPF      = '27512299800' --'91657954900'  
 --SET @DATAINI  = '20190801'   
 --SET @DATAFIM  = '20200417'   
 --SET @FORMATO  = 0  
 --SET @TIPO     = 0   
 /*---------------------------------------------*/   
    SET @ERRO = 0  
 ...
 DECLARE @RZ_OPERADORA CHAR(50)  
    ...
    IF @DATAINI > GETDATE() - 365  
 BEGIN  
    SET @DATAINI = CONVERT(CHAR(08), GETDATE() - 365, 112)  
 END  
 ...
 SELECT @QUERY =   
 'INSERT INTO #TRANSABT '                                                            + CHAR(13) +   
 'SELECT T.CODCLI, '                                                                 + CHAR(13) +    'SUBSTRING(dbo.MASCARACARTAO(T.CODCRT, 12), 7,6), '                                 + CHAR(13) +   
 'CASE T.NUMDEP WHEN 0 THEN ''T'' ELSE ''D'' END AS TD,'                             + CHAR(13) +   
 'T.CPF, '                                                                           + CHAR(13) +   
 'T.DATTRA,'                                                                         + CHAR(13) +   
 'T.NSUAUT, T.NSUHOS, T.VALTRA, '                                                    + CHAR(13) +   
  ...
  */  
  ...
  INSERT #LINREL(LINHAIMP, TIP) SELECT @BRANCOS, 2  
        ...
        INSERT INTO RELATORIO_RESULTADO  
          ...
          FROM #LINREL    /* saída do relatório */  
...
SET NOCOUNT OFF  
  ...
  WHERE NOMPROC = 'REL_HISTORICO_TRANSACOES'
  
  
/* PROCEDIMENTO PARA CRIAR O RELATORIO */  
/*  
SELECT * FROM RELATORIO WHERE NOMREL LIKE '%Historico das Transacoes por CPF%'  
SELECT * FROM RELATORIO WHERE ID_REL = 1930  
  
DECLARE @IDREL INT  
DECLARE @IDPAR INT  
SET @IDREL = 1930  
INSERT RELATORIO  
VALUES ( @IDREL, 'Historico das Transacoes por CPF', 'Historico das Transacoes por CPF', 'Transações', 'REL_HISTORICO_TRANSACOES', 'N', NULL, '*', 'S', NULL, 'NC', 'N')  
  
INSERT PARAMETRO VALUES( @IDREL  , @IDREL, 'CLIENTE'          , '@CODCLI'     , 'TextBox'      , 'String',  5, NULL,  'S', 'S', 1, 1)  
INSERT PARAMETRO VALUES( @IDREL+1, @IDREL, 'CPF    '          , '@CPF'        , 'TextBox'      , 'String', 11, NULL,  'N', 'N', 2, 2)  
INSERT PARAMETRO VALUES( @IDREL+2, @IDREL, 'DATA INICIO   '   , '@DATAINI'    , 'DateEdit'     , 'String', 10, NULL,  'N', 'N', 3, 3)  
INSERT PARAMETRO VALUES( @IDREL+3, @IDREL, 'DATA FIM   '      , '@DATAFIM'    , 'DateEdit'     , 'String', 10, NULL,  'N', 'N', 4, 4)  
INSERT PARAMETRO VALUES( @IDREL+4, @IDREL, 'DADOS CREDENCIADO', '@TIPO'       , 'DropDownList' , 'Byte'  ,  1, 0   ,  'S', 'S', 5, 5)  
INSERT PARAMETRO VALUES( @IDREL+5, @IDREL, 'SAÍDA PARA EXCEL ', '@FORMATO'    , 'CheckBox'     , 'Byte'  ,  1, NULL,  'S', 'N', 6, 6)  
  
SET @IDPAR = (SELECT MAX(ID) FROM DETALHES_PARAMETROS)  
INSERT DETALHES_PARAMETROS VALUES (@IDREL+4, 'NOME FANTASIA', 0, 'AlfaNumerico', @IDPAR+1)  
INSERT DETALHES_PARAMETROS VALUES (@IDREL+4, 'RAZAO SOCIAL ', 1, 'AlfaNumerico', @IDPAR+2)  
...
/* INSERE LISTAGEM NO MODULO DE RELATORIOS -- PARA O MODULO WEB*/  
/*  
DECLARE @ID_REL     INT  
DECLARE @ID_PAR     INT  
DECLARE @ID_DETALHE INT  
DECLARE @NOMPROC    VARCHAR(128)  
  
SET @NOMPROC = 'REL_HISTORICO_TRANSACOES'  
select @ID_REL = ID_REL from relatorio where NOMPROC = @NOMPROC  
  
DELETE D   
FROM DETALHES_PARAMETROS D                /* apaga os detalhes parametros se houverem */  
JOIN PARAMETRO P ON ( D.ID_PARAMETRO = P.ID_PAR)  
JOIN RELATORIO R ON ( P.ID_REL = R.ID_REL)  
WHERE R.ID_REL = @ID_REL  
  
DELETE PARAMETRO WHERE ID_REL = @ID_REL            /* apaga os parametros */  
DELETE RELATORIO WHERE ID_REL = @ID_REL            /* apaga o relatorio   */  
  
SET @ID_REL     = ( SELECT MAX(ID_REL) FROM RELATORIO)  + 1  
SET @ID_PAR     = ( SELECT MAX(ID_PAR) FROM PARAMETRO)  + 1  
SET @ID_DETALHE = ( SELECT MAX(ID)     FROM DETALHES_PARAMETROS) + 1  
  
INSERT RELATORIO VALUES ( @ID_REL, 'Historico das Transacoes por CPF', 'Historico Transacoes por CPF', 'Historico Transacoes',  @NOMPROC, 'N', NULL, '*', 'S', 'CLIENTE', '*', 'N', NULL)  
  
INSERT PARAMETRO VALUES( @ID_PAR  , @ID_REL, 'TIPO CARTAO'      , '@SISTEMA'    , 'TextBox'      , 'String',  5, NULL,  'S', 'S', 1, 1, NULL)  
INSERT PARAMETRO VALUES( @ID_PAR+2, @ID_REL, 'CPF    '          , '@CPF'        , 'TextBox'      , 'String', 11, NULL,  'N', 'N', 2, 2, NULL)  
INSERT PARAMETRO VALUES( @ID_PAR+3, @ID_REL, 'DATA INICIO (max.1 ano) '   , '@DATAINI'    , 'DateEdit'     , 'String', 10, NULL,  'N', 'N', 3, 3, NULL)  
INSERT PARAMETRO VALUES( @ID_PAR+4, @ID_REL, 'DATA FIM   '      , '@DATAFIM'    , 'DateEdit'     , 'String', 10, NULL,  'N', 'N', 4, 4, NULL)  
INSERT PARAMETRO VALUES( @ID_PAR+5, @ID_REL, 'DADOS CREDENCIADO', '@TIPO'       , 'DropDownList' , 'Byte'  ,  1, 0   ,  'S', 'S', 5, 5, NULL)  
INSERT PARAMETRO VALUES( @ID_PAR+6, @ID_REL, 'SAÍDA PARA EXCEL ', '@FORMATO'    , 'CheckBox'     , 'Byte'  ,  1, NULL,  'S', 'N', 6, 6, NULL)  
  
INSERT DETALHES_PARAMETROS VALUES (@ID_PAR ,      'POS PAGO', 0,     'Numerico', @ID_DETALHE  )  
INSERT DETALHES_PARAMETROS VALUES (@ID_PAR ,      'PRE PAGO', 1,     'Numerico', @ID_DETALHE+1)  
  
INSERT DETALHES_PARAMETROS VALUES (@ID_PAR +4, 'NOME FANTASIA', 0, 'AlfaNumerico', @ID_DETALHE +2)  
INSERT DETALHES_PARAMETROS VALUES (@ID_PAR +4, 'RAZAO SOCIAL ', 1, 'AlfaNumerico', @ID_DETALHE +3)  

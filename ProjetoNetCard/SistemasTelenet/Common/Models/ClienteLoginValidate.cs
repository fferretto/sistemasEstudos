...
{
    public sealed class ClienteLoginValidate : LoginValidateBase
    {
        public ClienteLoginValidate(ObjConn objConexao) 
            : base(objConexao)
        { }
        ...
        protected override LoginValidation OnValidate(IDataReader reader, Login login)
        {
            ...
            while (reader.Read())
            {
                ...
                OAuthAuthenticate($"{login.Codigo}-{login.LogIn}@{ObjConexao.CodOpe.ToString("000")}.cliente", login.Senha);

...
{
    public class ProcessaDadosCEF : IProcessaDadosCEF
    {
        ...
        public ProcessaDadosCEF(IPagNet_OcorrenciaRetPagService ocorrencia,
        ...
        public async Task<string> GeraArquivoCEF(BorderoPagVM model, int codArquivo)
        {
            try
            {
                string nmArquivo;
        ...
        public void GerarArquivoRemessa(int codArquivo, BorderoPagVM model, mdCedente cedente, string arquivo)
        {
            try
            {
                ...
                var ListaTitulosTED = TitulosPagamento.Where(x => x.PAGNET_CADFAVORECIDO.BANCO != "104" && x.TIPOTITULO == "TEDDOC").ToList();
                //var ListaTitulosDOC = listaFechamento.Where(x => x.FormaPGTO == "DOC" && x.CODBANCO != "0237").ToList();
                var ListaTitulosCC = TitulosPagamento.Where(x => x.PAGNET_CADFAVORECIDO.BANCO == "104" && x.TIPOTITULO == "TEDDOC").ToList();
        ...
        private string GeraTrailerLote(mdCedente cedente, BorderoPagVM model, IList<PAGNET_TITULOS_PAGOS> ListaPagamentos, string FormaPGTO, int NumLot, out int totalRegistroInserido)
        {
            ...
            foreach (var Titulo in ListaPagamentos)
            {
                ...
                if (FormaPGTO == "CC" && Titulo.PAGNET_CADFAVORECIDO.BANCO != "104")
                {

...
namespace PagNet.Application.Cnab.BancoBrasil
{
    public class CnabBrasil240
    {
        ...
        public string HeaderArquivo(mdCedente Cedente)
        {
            try
            {
                ...
                str += Geral.FormataInteiro("001", 3); //Código do Banco 
                ...
                str += Geral.FormataTexto(Cedente.DigitoAgencia, 1);   //Dígito Verificador da Agência
                ...
                str += Geral.FormataInteiro(ValorVazio, 1); //Dígito Verificador da Agência / Conta
                str += Geral.FormataTexto(Geral.RemoveCaracterEspecial(Cedente.nmOperadora.ToUpper()), 30); //Nome da Empresa
                str += Geral.FormataTexto("BANCO DO BRASIL S.A", 30); //Nome do Banco
                str += Geral.FormataTexto(ValorVazio, 10); //Uso Exclusivo FEBRABAN / CNAB 
                ...
                str += Geral.FormataTexto(ValorVazio, 20); //Para Uso Reservado do Banco 
                str += Geral.FormataTexto(ValorVazio, 20); //Uso Reservado da Empresa
                str += Geral.FormataTexto(ValorVazio, 14); //Uso Exclusivo FEBRABAN / CNAB
                str += Geral.FormataInteiro(ValorVazio, 15); //Uso Exclusivo FEBRABAN / CNAB
        ...
        public string HeaderLote(mdCedente Cedente, TransacaoTransferencia Transacoes, int NumLot)
        {
            try
            {
                ...
                str += Geral.FormataInteiro(Cedente.CodBanco.ToString(), 3); //Código do Banco 
                ...
                str += Geral.FormataTexto(ValorVazio, 2); //Indicativo da Forma de Pagamento do Serviço 
                str += Geral.FormataTexto(ValorVazio, 6); //Uso Exclusivo FEBRABAN/CNAB  
                str += Geral.FormataInteiro(ValorVazio, 10); //Ocorrências para o Retorno
        ...
        public string HeaderLotePagBoleto(mdCedente Cedente, TransacoesPagamento Transacoes, int NumLot)
        {
            try
            {
                ...
                str += Geral.FormataInteiro("001".ToString(), 3); //Código do Banco 
                str += Geral.FormataInteiro(NumLot.ToString(), 4); //Lote de Serviço
                str += Geral.FormataInteiro("1", 1); //Tipo de Registro
                str += Geral.FormataTexto("C", 1); //Tipo da Operação
                str += Geral.FormataInteiro(Transacoes.TipoServico.ToString(), 2); //Tipo de Serviço
        ...
        public string DetalheLoteSegmento_A(mdCedente Cedente, TransacaoTransferencia Transferencia, int codCamaraCentralizadora, int NumLot, int NumSeq)
        {
            try
            {
                ...
                str += Geral.FormataTexto(ValorVazio, 20); //Nro. do Documento Banco
        ...
        public string DetalheLoteSegmento_B(mdCedente Cedente, TransacaoTransferencia Transferencia, int NumLot, int NumSeq)
        {
            try
            {
                ...
                str += Geral.FormataInteiro(Geral.TrataValorMonetario(Transferencia.Valor), 15); //Valor do Documento

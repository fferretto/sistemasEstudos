...
using PagNet.Bld.PGTO.Safra.ModelAuxiliar;
using PagNet.Bld.PGTO.Safra.Util;
using System;
using System.Collections.Generic;
using System.Text;

namespace PagNet.Bld.PGTO.Safra.CNAB
{
    public class Cnab240
    {
        ...
        public string HeaderArquivo(mdCedente Cedente)
        {
            try
            {
                ...
                str += Helper.FormataInteiro("422", 3); //Código do Banco na Compensação 
                ...
                str += Helper.FormataTexto(Cedente.DigitoAgencia, 1); //Dígito Verificador da Agência / Conta
                str += Helper.FormataTexto((Helper.RemoveCaracterEspecial(Cedente.nmOperadora)).ToUpper(), 30); //Nome da Empresa
                str += Helper.FormataTexto("SAFRA", 30); //Nome do Banco
        ...
        public string TrailerArquivo(int TotaLote, int TotalReg, int qtContas)
        {
            ...
            {
                string str = ValorVazio;
                str += Helper.FormataInteiro("422", 3); //Código do Banco 
        ...
        public string HeaderLote(mdCedente Cedente, TransacaoTransferencia Transacoes, int NumLot)
        {
            try
            {
                ...
                str += Helper.FormataTexto(Cedente.DigitoAgencia.ToString(), 1); //Dígito Verificador da Agência/Conta

...
namespace PagNet.Bld.Domain.Services
{
    public class PAGNET_EMISSAO_TITULOSService : ServiceBase<PAGNET_EMISSAO_TITULOS>, IPAGNET_EMISSAO_TITULOSService
    {
        ...
        {
            var dados = _log.Get(x => x.CODTITULO == CODTITULO, "PAGNET_CADFAVORECIDO,PAGNET_CADEMPRESA,PAGNET_USUARIO")
        ...
        public void CancelaTituloHomologacao(int codigoContaCorrente, int codigoUsuario, string Justificativa)
        {
            var titulo = _rep.Get(x => x.CODCONTACORRENTE == codigoContaCorrente && 
                                       x.STATUS != "CANCELADO" && 
                                       x.STATUS != "BAIXADO" && 
                                       x.TIPOTITULO == "HOM").FirstOrDefault();

            titulo.STATUS = "CANCELADO";
            _rep.Update(titulo);

            IncluiLog(titulo, codigoUsuario, Justificativa);
        }
        public PAGNET_EMISSAO_TITULOS RetornaTituloHomologacao(int codigoContaCorrente)
        {
            var titulo = _rep.Get(x => x.CODCONTACORRENTE == codigoContaCorrente &&
                                       x.STATUS != "CANCELADO" &&
                                       x.STATUS != "BAIXADO" &&

...
using PagNet.Bld.PGTO.Brasil.ModelAuxiliar;
using PagNet.Bld.PGTO.Brasil.Util;
using System;

namespace PagNet.Bld.PGTO.Brasil.CNAB
{
    public class Cnab240
    {
        ...
        public string HeaderArquivo(mdCedente Cedente)
        {
            try
            {
                ...
                str += Helper.FormataInteiro("001", 3); //Código do Banco 
                ...
                str += Helper.FormataTexto(Cedente.DigitoAgencia, 1);   //Dígito Verificador da Agência
                ...
                str += Helper.FormataInteiro(ValorVazio, 1); //Dígito Verificador da Agência / Conta
                str += Helper.FormataTexto(Helper.RemoveCaracterEspecial(Cedente.nmOperadora.ToUpper()), 30); //Nome da Empresa
                str += Helper.FormataTexto("BANCO DO BRASIL S.A", 30); //Nome do Banco
                str += Helper.FormataTexto(ValorVazio, 10); //Uso Exclusivo FEBRABAN / CNAB 
                ...
                str += Helper.FormataInteiro("103", 3); //Número da Versão do Layout
                str += Helper.FormataInteiro("0", 5); //Densidade de Gravação Arquivo
                str += Helper.FormataTexto(ValorVazio, 20); //Para Uso Reservado do Banco 
                str += Helper.FormataTexto(ValorVazio, 20); //Uso Reservado da Empresa
                str += Helper.FormataTexto(ValorVazio, 14); //Uso Exclusivo FEBRABAN / CNAB
                str += Helper.FormataInteiro(ValorVazio, 15); //Uso Exclusivo FEBRABAN / CNAB
        ...
        public string HeaderLote(mdCedente Cedente, TransacaoTransferencia Transacoes, int NumLot)
        {
            try
            {
                ...
                str += Helper.FormataInteiro(Cedente.CodBanco.ToString(), 3); //Código do Banco 
                ...
                str += Helper.FormataInteiro("046", 3); //Número da Versão do Lote
                ...
                str += Helper.FormataTexto(ValorVazio, 2); //Indicativo da Forma de Pagamento do Serviço 
                str += Helper.FormataTexto(ValorVazio, 6); //Uso Exclusivo FEBRABAN/CNAB  
                str += Helper.FormataInteiro(ValorVazio, 10); //Ocorrências para o Retorno
        ...
        public string HeaderLotePagBoleto(mdCedente Cedente, TransacoesPagamento Transacoes, int NumLot)
        {
            try
            {
                ...
                str += Helper.FormataInteiro("001".ToString(), 3); //Código do Banco 
                str += Helper.FormataInteiro(NumLot.ToString(), 4); //Lote de Serviço
                str += Helper.FormataInteiro("1", 1); //Tipo de Registro
                str += Helper.FormataTexto("C", 1); //Tipo da Operação
                str += Helper.FormataInteiro(Transacoes.TipoServico.ToString(), 2); //Tipo de Serviço
        ...
        public string DetalheLoteSegmento_A(mdCedente Cedente, TransacaoTransferencia Transferencia, int codCamaraCentralizadora, int NumLot, int NumSeq)
        {
            try
            {
                ...
                str += Helper.FormataTexto(ValorVazio, 20); //Nro. do Documento Banco
        ...
        public string DetalheLoteSegmento_B(mdCedente Cedente, TransacaoTransferencia Transferencia, int NumLot, int NumSeq)
        {
            try
            {
                ...
                str += Helper.FormataInteiro(Helper.TrataValorMonetario(Transferencia.Valor), 15); //Valor do Documento
